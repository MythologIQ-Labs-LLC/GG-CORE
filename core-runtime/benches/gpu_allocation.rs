...
fn bench_rapid_alloc_burst(c: &mut Criterion) {
    ...
    for &count in &[10, 100, 500] {
        group.bench_with_input(BenchmarkId::new("burst", count), &count, |b, &n| {
            let allocator = MockGpuAllocator::new(n * 4096 + 4096, 0);
            b.iter(|| {
                let allocs: Vec<_> = (0..n).map(|_| allocator.allocate(4096).unwrap()).collect();
                for a in &allocs {
...
fn bench_pool_allocation(c: &mut Criterion) {
    ...
    for &size in &[1024_u64, 4096, 65536] {
        group.bench_with_input(BenchmarkId::new("pool_alloc", size), &size, |b, &sz| {
            b.iter_with_setup(
                || {
...
fn bench_pool_contention(c: &mut Criterion) {
    ...
    for &threads in &[2, 4, 8] {
        group.bench_with_input(BenchmarkId::new("threads", threads), &threads, |b, &n| {
            b.iter(|| {
                let allocator: Arc<dyn GpuAllocator> = Arc::new(MockGpuAllocator::new(1 << 30, 0));
...
fn bench_raii_drop(c: &mut Criterion) {
    ...
    for &size in &[1024_usize, 1 << 20] {
        group.bench_with_input(BenchmarkId::new("drop_cost", size), &size, |b, &sz| {
            let device = Arc::new(GpuDevice::cpu());
            let allocator: Arc<dyn GpuAllocator> = Arc::new(MockGpuAllocator::new(1 << 30, 0));
            b.iter(|| {
                let alloc = allocator.allocate(sz).unwrap();
                let mem = GpuMemory::new_allocated(device.clone(), alloc, allocator.clone());
                drop(black_box(mem));
            });
        });
    }
