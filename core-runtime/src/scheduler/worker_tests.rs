...
async fn setup() -> (Arc<RequestQueue>, Arc<InferenceEngine>) {
    ...
    engine
        .register_model("test-model".into(), handle, MockModel::arc("test"))
        .await;
    (queue, engine)
}
...
async fn worker_shuts_down_gracefully() {
    ...
    let result = tokio::time::timeout(std::time::Duration::from_secs(2), worker).await;
...
async fn worker_rejects_when_memory_limit_exceeded() {
    ...
    assert!(
        result.is_err(),
        "request should be rejected due to memory limit"
    );
...
async fn worker_rejects_when_concurrency_limit_exceeded() {
    ...
    assert!(
        result.is_err(),
        "request should be rejected due to concurrency limit"
    );
    let msg = result.unwrap_err();
    assert!(
...
async fn worker_releases_resource_guard_after_inference() {
    ...
    let (_id1, rx1) = queue
        ...
        .await
        .unwrap();
    let r1 = tokio::time::timeout(std::time::Duration::from_secs(2), rx1)
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        limits.current_concurrent(),
        0,
        "guard should have been dropped"
    );
    ...
    assert!(
        r2.is_ok(),
        "second request should succeed after guard is released"
    );
...
async fn admission_rejection_propagates_to_caller() {
    ...
    let channel_result = tokio::time::timeout(std::time::Duration::from_secs(2), rx).await;
    ...
    assert!(
        ...
        "caller must receive the rejection error"
    );
...
async fn streaming_enqueue_succeeds_with_capacity() {
    ...
    assert!(
        result.is_ok(),
        "streaming enqueue should succeed with capacity"
    );
}
...
async fn streaming_enqueue_rejects_when_queue_full() {
    ...
    assert!(
        result.is_err(),
        "streaming enqueue must fail when queue is full"
    );
}
