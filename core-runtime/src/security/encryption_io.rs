...
pub fn read_file_bytes(path: &Path) -> Result<Vec<u8>, EncryptionError> {
    let mut file =
        std::fs::File::open(path).map_err(|e| EncryptionError::IoError(e.to_string()))?;
...
pub fn write_encrypted_file(path: &Path, nonce: &[u8], ct: &[u8]) -> Result<(), EncryptionError> {
    let mut out =
        std::fs::File::create(path).map_err(|e| EncryptionError::IoError(e.to_string()))?;
    out.write_all(b"GGGCM")
        .map_err(|e| EncryptionError::IoError(e.to_string()))?;
    out.write_all(&[2, 0])
        .map_err(|e| EncryptionError::IoError(e.to_string()))?;
    out.write_all(nonce)
...
fn read_gcm_payload(
    ...
    let mut ciphertext = vec![0u8; len]; // codeql[rust/hard-coded-cryptographic-value] zero-init read buffer, not a key
