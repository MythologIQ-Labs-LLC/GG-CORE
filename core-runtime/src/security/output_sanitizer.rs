...
mod tests {
    ...
    fn test_performance() {
        let sanitizer = OutputSanitizer::default_sanitizer();
        let output = "Contact support@example.com for help. Call 555-123-4567. SSN: 123-45-6789."
            .repeat(100);
    ...
    fn test_streaming_pii_split_attack() {
        ...
        assert!(
            full_buffer.contains("john.smith@example.com") || full_buffer.contains("[REDACTED"),
            "Buffer should contain either the full email or redacted version"
        );

        // Verify phone number is also tracked
        assert!(
            full_buffer.contains("555-123-4567") || full_buffer.contains("[REDACTED"),
    ...
    fn test_safe_trim_point() {
        ...
        assert!(
            buffer.len() > 100,
            "Buffer must be longer than MAX_PII_LENGTH"
        );
        ...
        assert!(
            remaining >= 100,
            "Remaining buffer should be at least MAX_PII_LENGTH"
        );
    }
