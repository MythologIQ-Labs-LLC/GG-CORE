...
pub fn has_excessive_repetition(text: &str) -> bool {
    ...
    // Optimization: Use `&[&str]` slices as keys instead of joining them into `String`s.
    // Slices implement `Hash` and `Eq` natively, avoiding O(N * window_size) heap allocations.
