...
| B-29b | ONNX registry unification (epic): unified GGUF/ONNX model abstraction so the engine registry (`Arc<dyn GgufModel>`) can hold ONNX models, wiring `load_onnx_from_manifest` into the prod load sites (`ffi/models.rs`, `python/session.rs`) for end-to-end ONNX inference | issue #72 (follow-up) | P2 | open | The real end-to-end enabler; L3. B-29a dispatch is unreachable in production until this lands (research brief F2) |
