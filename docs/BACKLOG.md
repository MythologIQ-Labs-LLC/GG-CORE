...
| B-28 | ONNX classifier scope-2: replace naive `simple_tokenize` with a real WordPiece/subword tokenizer (`tokenizers` crate) for the ONNX classify/embed paths | issue #72 (follow-up) | P2 | open | Resequenced earlier (Entry #117): the real tokenizer is a shared dependency for B-24b's faithful detokenization. Plan under its own gate |
