...
| B-25b | FFI/Python inference reroute through `Runtime::infer/infer_stream` (fix the enqueue-then-await-no-worker deadlock; map `SecurityRejected`; gguf-gate streaming) | research brief 2026-07-26 (Entry #104) | P1 | open | Deferred L3 cycle; reroute `core_infer`/`core_infer_streaming`/Python `Session::infer` onto the secure façade once the deadlock fix is designed under its own `/qor-audit` |
