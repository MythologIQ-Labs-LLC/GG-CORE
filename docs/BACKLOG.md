...
| B-29b-2 | Manifest loading + architecture dispatch: load `ModelManifest` in the prod load path (`ffi/models.rs`, `python/session.rs`); branch on `ModelArchitecture` to call `load_onnx_from_manifest` (B-29a); reconcile `ModelMetadata`/`ModelManifest` | issue #72 (follow-up) | P2 | open | The step that makes ONNX servable end-to-end. `load_metadata` currently returns name+size only (research brief F3); L2 |
