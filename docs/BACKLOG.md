...
| B-25 | Consumable FFI/Python surface: secure inference reroute. FIRST add CI feature legs (gguf/python/ffi) to rust.yml, then remediate pre-existing FFI defects (ffi/inference.rs 272-line Razor extraction; core_infer_bounded deadlock), then reroute core_infer/core_infer_streaming/Python Session::infer to Runtime::infer/infer_stream | audit Entry #101/#102, Shadow Genome #7 | P1 | open | Add CI legs first (verifiable ground) |
