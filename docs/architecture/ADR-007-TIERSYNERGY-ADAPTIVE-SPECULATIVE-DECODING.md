...
**Status:** Accepted — implemented (dormant); see [Implementation Status & Consolidation](#implementation-status--consolidation-2026-07-31-b-21)  
**Date:** 2026-07-08 (proposed); 2026-07-31 (adopted to `main` + reconciled, B-21a)  
...
## Implementation Status & Consolidation (2026-07-31, B-21)

This ADR was authored 2026-07-08 as **Proposed** on an unmerged branch (PR #59). The design was
subsequently **implemented and sealed** as eight governed cycles (GitHub issues #61–#68, META_LEDGER
entries #87–#94) surfacing as features **F-48–F-54** in `docs/FEATURE_INDEX.md`. This section (added
when the ADR was adopted onto `main`, B-21a) reconciles the doc with the built reality; where it and
the older `ADR-007-CONSOLIDATION-AUDIT.md` disagree, this section supersedes (that audit predates
`adaptive_speculative` and did not see the redundancy below).

### Built, sealed — but dormant
Every issue is implemented and unit-tested (config, decoder interfaces, heuristic scheduler,
TierSynergy plan selector, telemetry/auto-disable, security tests, benchmark). **However the entire
speculative surface is built-but-dormant behind the `advanced` feature gate: nothing is wired into
the production inference path** — `engine/inference.rs` and `Runtime::infer` contain zero speculative
references; the decoders/plan/traits are instantiated only in tests. The current benchmark measures
construction/selection overhead only (synthetic inputs), not end-to-end speedup. Turning speculation
on in production is tracked as **B-21c** (see below).

### Canonical implementation + consolidation (triple → single)
Three parallel implementations currently coexist, none wired to production:
- `core-runtime/src/engine/speculative.rs` (**v1**) — simplest token-level decoder; owns the only
  GGUF backend adapter (`engine/gguf/speculative.rs` implements v1's `DraftModel`/`TargetModel`).
- `core-runtime/src/engine/speculative_v2.rs` (**v2**) — richer token-level decoder (rejection
  sampling, `SpeculativeStats`); its `SpeculativeConfig`/`SpeculativeStats` are used by
  `models/tier_synergy/`. No GGUF backend.
- `core-runtime/src/engine/adaptive_speculative/` (**canonical**) — the ADR-007 block-level adaptive
  design (traits + heuristic + telemetry). No concrete executor yet.

**`adaptive_speculative/` is the canonical, go-forward implementation.** v1 and v2 retire on the
operator-confirmed, dependency-sound sequence (each a governed cycle):
- **B-21a** (this doc) — canonical ADR on `main` + backlog reconciliation.
- **B-21b-1** — retire **v1**: port the GGUF adapter to v2's traits, migrate F-18, delete
  `speculative.rs` (triple → double).
- **B-21c** — build the `adaptive_speculative` executor (absorbing v2's rejection-sampling
  mechanics) + a GGUF backend for the adaptive traits, and **wire it into `Runtime::infer`**
  (prompt-injection scan BEFORE, output sanitize AFTER speculation; rejected tokens never committed;
  single-model fallback the default; auto-disable honored). L3.
- **B-21b-2** — retire **v2**: migrate TierSynergy's config/stats onto the adaptive path; delete
  `speculative_v2.rs` (double → single).
- **B-21e** — replace the overhead-only benchmark with a real speculative-vs-baseline e2e matrix.
- **B-21d** — loose ends: audit follow-ups F2/F3; add the `t1_draft_model_path_enforces_allowlist`
  security test that `docs/security/THREAT_MODEL.md` §12.2 cites but which is currently absent.

Grounding for this section: `docs/research-brief-b21-adr007-scoping-2026-07-31.md` (ledger #180).
